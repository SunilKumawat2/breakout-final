...
const page = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = async () => {
      // 🔥 wait for full render (important)
      await new Promise((res) => setTimeout(res, 800));
      if (!isMounted) return;
      let targetId = null;
      if (
        sessionStorage.getItem("scrollToEscapeRooms") === "true" &&
        location?.escaperooms?.extreme?.length > 0
      ) {
        targetId = "escape-rooms-section";
        sessionStorage.removeItem("scrollToEscapeRooms");
      } else if (
        sessionStorage.getItem("scrollToEscapeRooms_ultra") === "true" &&
        location?.escaperooms?.ultra?.length > 0
      ) {
        targetId = "escape-rooms-ultra";
        sessionStorage.removeItem("scrollToEscapeRooms_ultra");
      } else if (
        sessionStorage.getItem("location_know_more") === "true" &&
        location?.imagecardssection?.card?.length > 0
      ) {
        targetId = "loction-know-more-section";
        sessionStorage.removeItem("location_know_more");
      }
      ...
      // 🔥 ensure element is in DOM properly
          ...
          <>
            ...
            <div className="black-gr-div">
              {location && location?.textsection?.description != "" && (
                <HmTextSec className="pt-80"
                  text={location?.textsection?.description} />
              )}
              {location && location?.escaperooms?.extreme?.length > 0 && (
                <section className="esc-sec pt-80" id="escape-rooms-section">
                  <div className="container">
                    ...
                    <div className="row row-gap-25">
                      {location?.escaperooms?.extreme
                        ?.filter((item) => {
                          // Apply hide condition only for Koramangala
                          if (location?.title == "Koramangala") {
                            return (
                              item?.slug != "hostage" &&
                              item?.slug != "classified" &&
                              item?.slug != "bomb-defusal" &&
                              item?.slug != "prison-break"
                            );
                          }

                          // For other locations show all games
                          return true;
                        })
                        ?.map((item, index) => (
                          <div
