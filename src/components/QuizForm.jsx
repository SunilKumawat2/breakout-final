...
console.log("resource_resource_resource",resource)
  const IGNORE_VALUES = [,
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        /* ================= MAILCHIMP API ================= */
        try {
          await axios.post("/api/mailchimp_resource", {
            name: values.name,
            phone: values.phone,
            email: values.email || "",
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      console.log("checking Total_venues 👉", storedTotal);

      if (storedTotal) {
        setTotalVenues(Number(storedTotal));
        ...
        <div className="row">
          <div className="col-lg-12 text-center">
            ...
            <h2 className="sec-head medium mt-5">
              Enter Details to <span>Unlock Venues</span>
            </h2>
          </div>
        </div>
