...
const Step1 = ({ goToResult, content }) => {
...
`;
  ...
  const handleAnswer = (value, optionId = null) => {
    ...
    // ✅ ONLY for input type (slider)
    if (question?.question_type == "input") {
      const range = getRangeConfig(question);
      finalValue = formatValue(value, range?.max); // 👈 USE THIS
    }
    ...
    const updatedAnswers = {
      ...
      [key]: {
        ...
        value: finalValue, // ✅ now "100+" will be stored
        option_ids: optionId
    ...
    console.log("👉 Final Saved Value:", finalValue);
